(function ($) {
    ...
    var getUrl = '';
    if (sub) {
        getUrl = 'http://search.china.org.cn/hlftiweb/en/jsonfn.jsp?strUrl=englishscio&page=' + startpage + '&nItem=' + pageSize + '&sub=' + sub;
    } else {
        getUrl = 'http://search.china.org.cn/hlftiweb/en/jsonfn.jsp?strUrl=englishscio&page=' + startpage + '&nItem=' + pageSize + '&searchText=' + searchText;
    }
    ...
    window.searchRes = function (data) {
        ...
        for (var i = 0; i < data.list.length; i++) {
            temp.push('<li><h3><span class="mmmm">' + data.list[i].sub + '</span>' + data.list[i].date + '</h3><h1><a href="' + data.list[i].url.replace(/http:\/\/www.china.org.cn\/englishscio\//, "http://english.scio.gov.cn/") + '">' + data.list[i].title + '</a></h1><p>' + data.list[i].text + '</p></li>');
        }
    ...
    function pageObj() {
        ...
        var pagination_buf = [];
        // prev
        pagination_buf.push(startpage > 1 ? '<a href="?strUrl=englishscio&page=' + (startpage - 1) + '&' + key + '=' + txt + '">Prev</a>' : '<span>Prev</span>');
        // pagination
        if (totalPage <= 10) {
            for (i = 1; i <= totalPage; i++) {
                pagination_buf.push(i == startpage ? '<span>' + i + '</span>' : '<a href="?strUrl=englishscio&page=' + i + '&' + key + '=' + txt + '">' + i + '</a>');
            }
        } else {
            // total page > 10
            if (startpage <= 4) {
                ...
                for (i = totalPage - 2; i <= totalPage; i++) {
                    pagination_buf.push('<a href="?strUrl=englishscio&page=' + i + '&' + key + '=' + txt + '">' + i + '</a>');
                }
        ...
        // next
        pagination_buf.push(startpage < totalPage ? '<a href="?strUrl=englishscio&page=' + (startpage + 1) + '&' + key + '=' + txt + '">Next</a>' : '<span>Next</span>');
    ...
    function keyHref() {
        ...
        $('.mmmm').each(function () {
            ...
            $(this).addClass(arr[curTitle]).html('<a href="?strUrl=englishscio&sub=' + curTitle + '" target="_top">' + curTitle + '</a>')
        });
    }
})(jQuery);
